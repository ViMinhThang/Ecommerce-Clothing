    ...
    <div className="border rounded-xl overflow-hidden shadow-sm bg-background">
      ...
      >
        {columns.map((col) => (
          <div
            key={String(col.key)}
            className={`flex items-center gap-2 px-4 py-3 hover:bg-secondary ${
      ...
      {sortedData.map((item, i) => (
        ...
        >
          ...
          <div className="flex items-center gap-4 px-4 py-3">
            <Bolt
              className="cursor-pointer text-slate-500 dark:text-slate-400 hover:text-blue-500"
              onClick={() => onEdit?.(item, "edit")}
            />
            <Trash
              className="cursor-pointer text-slate-500 dark:text-slate-400 hover:text-red-500"
